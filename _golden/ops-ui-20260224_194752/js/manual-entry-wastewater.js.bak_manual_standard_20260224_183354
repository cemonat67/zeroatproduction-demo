...
document.addEventListener("click", async (e)=>{
  ...
  // Close (X / Close button / backdrop)
  if (t.id === "wwClose" || t.id === "wwCloseX" || (t.classList && t.classList.contains("ww-backdrop"))){
    e.preventDefault();
  ...
  if (t.id === "wwSave"){
    e.preventDefault();
    try{
      // call the local function if it exists in this IIFE scope
      if (typeof saveToSupabase === "function") {
        await saveToSupabase();
      } else {
        console.warn("[WW] saveToSupabase() not in scope");
      }
    }catch(err){
      console.warn("[WW] save click failed", err);
    }
    return;
  }
}, true);
...
// __WW_BIND_MANUAL_V1__
// Purpose: bind WW modal to current UI triggers: #tabManual and #btnOpenManualModal
(function(){
  function bind(){
    var open = window.__WW_openModal || (typeof openModal === "function" ? openModal : null);
    if(!open){
      // openModal is in module scope; try delayed bind
      return false;
    }

    function hook(id){
      var el = document.getElementById(id);
      if(!el || el.__WW_BOUND) return;
      el.__WW_BOUND = true;

      el.addEventListener("click", function(e){
        e.preventDefault();
        try{ (window.__WW_openModal || open)(); }
        catch(err){ console.warn("[WW] open failed", err); }
      }, true);

      console.log("[WW] bound to #" + id + " (manual triggers)");
    }

    hook("tabManual");
    hook("btnOpenManualModal");
    return true;
  }
  ...
  (function loop(){
    ...
    if(bind()) return;
    if(tries > 80){
      console.warn("[WW] bind timeout (manual triggers)");
      return;
    }
    setTimeout(loop, 50);
  })();
